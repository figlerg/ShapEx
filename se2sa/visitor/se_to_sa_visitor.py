from se2sa.automaton.alphabet.const_letter import ConstLetter
from se2sa.automaton.alphabet.exp_letter import ExpLetter
from se2sa.automaton.alphabet.line_letter import LineLetter
from se2sa.automaton.alphabet.sinc_letter import SincLetter
from se2sa.automaton.alphabet.sine_letter import SineLetter
...
from se2sa.generated.ShapeExpressionVisitor import ShapeExpressionVisitor
from se2sa.se2sa import *

from nonlin.nonlin2sympy import str2sympy
from se2sa.eliminate_var_nonlin import eliminate_var_nonlin
...
class SEToSAVisitor(ShapeExpressionVisitor):
    def visitShape_expression(self, ctx: ShapeExpressionParser.Shape_expressionContext):
        ...
        for i in range(ctx.getChildCount()):
            if isinstance(ctx.children[i], ShapeExpressionParser.Param_declarationContext):
                ...
                tuples.append(pair)
                # nondiscrete case
            if isinstance(ctx.children[i], ShapeExpressionParser.Discrete_param_declarationContext):
                pair = self.visit(ctx.discrete_param_declaration(nr_of_discrete_param))
    ...
    def visitRelation_string(self, ctx:ShapeExpressionParser.Relation_stringContext):
        ...
        # return str2sympy(ctx.getText()[6:-1]) # return normal sympy expression
