...
def str2sympy(raw_text:str):
    ...
    # the constraint indeed has exactly one inequality
    rel_types = ['<=', '>=', '<', '>']
    found = {}
    tmp = raw_text
    for rel in rel_types:
        ...
        assert len(idx) <= 1, "Chained inequalities not allowed in nonlin constraint."
        if idx:
    ...
    assert len(found) == 1, "Nonlinear constraint '" + raw_text +\
