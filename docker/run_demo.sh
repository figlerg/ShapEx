...
docker rm $(docker stop $(docker ps -a -q --filter ancestor=shapex:latest --format="{{.ID}}")) >/dev/null
...
docker run -p 8888:8888 -d -t shapex:latest /bin/bash -c "/opt/conda/bin/jupyter notebook --ip=0.0.0.0 --port=8888 --no-browser --allow-root --NotebookApp.token='0291029398729374728jdaojalklyo'" >/dev/null

echo "Started docker container and jupyter server"

sleep 0.5s # give container some time to start jupyter
...
echo "Try opening the URL: http://127.0.0.1:8888/notebooks/demo.ipynb?token=0291029398729374728jdaojalklyo"
